    ...
    const fetchCandidates = async () => {
      try {
        ...
        const selectedCandidates = (data.Candidate || [])
          ...
          .filter(c => c.candidate_status === "archived"); // Keep only selected
...
const handleUnarchived = (candidateId) => {
  setCandidates(prev =>
    prev
      .map(c =>
        c.CandidateID === candidateId
          ? { ...c, candidate_status: "onhold" } // update status
          : c
      )
      .filter(c => c.candidate_status === "archived") // remove non-archived
  );
};
